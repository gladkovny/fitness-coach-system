...
Кабинет тренера    ✅ Работает  v1.0 (6 вкладок)
...
─────────────────────────────────────
Тестирование Марк  🔄 День 48 из 90 (hybrid)
...
## 🟢 МИГРАЦИЯ НА SUPABASE — ТЕКУЩИЙ ЭТАП (февраль 2026)

**Актуальный статус для синхронизации с Claude:** см. **docs/SYNC_STATUS.md**. **Правила для AI (7 разделов, 2 контура):** **docs/cursorrules_v2.1.md** (Cursor), **docs/CLAUDE_RULES_V2.1.md** (Claude). **Экосистема трёх репо (CRM + ФитПрога + exercise-video-bot):** **CLAUDE.md** в корне.
...
| Кабинет тренера | deploy/master/cabinet — Supabase; профиль (только данные клиента), сессии+блоки объединены, противопоказания/ограничения, рост из анкеты; migrateToSupabase('profile') для backfill |
| Инструкция первого запуска | supabase/ИНСТРУКЦИЯ_ПЕРВЫЙ_РАЗ.md, TESTING_FIRST_TIME.md |

**Где застряли:** нет критических блокеров. При отсутствии Node: `cd deploy\master` → `python -m http.server 3000` или Live Server.

**Следующие шаги (24.02.2026):**  
1) Стабилизация Unified Tracker v4.4, фикс багов по фидбеку Ярослава и Кирилла.  
2) Онбординг — завершить тест на Алене, автозаполнение Goals.  
3) Дашборд и программа Марка — пока на GAS. Подробнее: **docs/SYNC_STATUS.md**.
...
- [x] Рост из анкеты → Supabase (маппинг формы, MigrateToSupabase profile)
- [ ] Тестирование на Алене — завершить цикл
- [ ] Автозаполнение Goals из формы

### Приоритет 2.1: Кабинет тренера (✅ завершён 17.02)
- [x] Профиль: только данные клиента, добавлены противопоказания и ограничения
- [x] Сессии и Блоки объединены в одну вкладку
- [x] Рост подтягивается из анкеты (ClientProfile/Form/Goals), migrateToSupabase('profile') для backfill
...
|------|-----------|--------|-----------|
| 24.02.2026 | CLAUDE.md | — | Экосистема трёх проектов: fitness-coach-system, Telegram_Mini_App (@fitprogabot), exercise-video-bot (@gif_converter_bot); общая Supabase, синхронизация exercises |
| 24.02.2026 | Supabase | migrations | 00018_training_blocks_exercise_template.sql (шаблон упражнений в блоках) |
| 18.02.2026 | Cabinet (тренер) | 1.0 | Кабинет тренера: список клиентов, карточка клиента (6 вкладок: Обзор, Тренировки, Тело и здоровье, Питание, Календарь, Дашборд клиента), управление видимостью дашборда клиента |
| 17.02.2026 | Кабинет тренера | — | Профиль (только данные клиента), сессии+блоки, противопоказания/ограничения, рост из анкеты |
