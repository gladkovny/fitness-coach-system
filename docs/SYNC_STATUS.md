...
| Трекер тренера | 📋 | Следующий — подключение к Supabase |
| Дашборд/программа Марка | 📋 | Пока GAS, опционально — миграция на Supabase |
...
| 2 | **Подключить трекер к Supabase** | deploy/master/tracker (index.html, assessment.html) — перевести загрузку/сохранение с GAS на Supabase, использовать единый выбор клиента после входа. |

### Средний срок

| # | Задача | Детали |
|---|--------|--------|
| 3 | **Дашборд Марка** | deploy/mark/dashboard — опционально перевести чтение на Supabase по client_id Марка; пока может оставаться на Master API (GAS). |
| 4 | **Программа Марка** | deploy/mark/program — долгосрочно: перенести программы A/B/C и логи в Supabase, заменить markWorkoutApiUrl. |
...
Кратко: «Проект fitness-coach-system. Сейчас этап: миграция на Supabase. Вход и дашборд master уже на Supabase; локальный запуск застрял — npx и python не найдены. Обход: Live Server (Cursor) или установка Node.js. Нужно: 1) проверить вход и дашборд, 2) дальше — трекер на Supabase. Контекст: docs/SYNC_STATUS.md, supabase/ИНСТРУКЦИЯ_ПЕРВЫЙ_РАЗ.md.»
