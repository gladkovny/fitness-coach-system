---
name: Fitness Coach System Rules
description: Правила разработки для проекта SaaS-системы фитнес-тренеров. Включает код-стайл, протоколы решения проблем, структуру проекта и специфику распознавания упражнений.
---

# Fitness Coach System — Правила разработки

## 📋 О проекте
Коробочная SaaS-система для фитнес-тренеров. Владелец: Николай (не программист, работает с AI).

**Ключевые документы:**
- Полные правила: `docs/cursorrules_v2.1.md`
- Правила для Claude: `docs/CLAUDE_RULES_V2.1.md`
- Текущий статус: `docs/SYNC_STATUS.md`
- Обзор проекта: `docs/PROJECT_OVERVIEW_AND_RULES.md`

---

## 🏗️ Два контура системы

| Контур | Деплой | Стек | Страницы |
|--------|--------|------|----------|
| **Master** | `deploy/master` | Supabase | login, dashboard, tracker |
| **Mark** | `deploy/mark` | GAS | dashboard, program |

> Идёт переход на Supabase (Фаза 2)
...
└── js/               — Общие модули (api.js, utils.js)
...
1. Затронутые разделы [1]–[7]
2. Контур (Master или Mark)
3. Риски
4. Чек-лист проверки

**После завершения:**
- Напоминать про коммит: `.\\scripts\\git-push.ps1`
- При необходимости: `.\\scripts\\sync-claude.ps1`

---

## 🧪 Pre-commit и тесты

- **Husky + lint-staged** при `git commit`:
  - ESLint --fix и Prettier для `*.js`
  - Prettier для `*.html`
  - `npm test`
- Тесты распознавания: `npm test`
