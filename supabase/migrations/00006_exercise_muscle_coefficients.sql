-- Добавляем muscle_coefficients для расчёта нагрузки по мышцам
-- Fallback: category/subcategory → коэффициенты
-- Дата: 2026-01-31

-- Обновляем упражнения с пустыми коэффициентами по категории
UPDATE exercises e
SET muscle_coefficients = COALESCE(
  CASE 
    WHEN e.category ILIKE '%рудь%' OR e.subcategory ILIKE '%грудь%' THEN '{"chest": 0.8, "front_delt": 0.15, "triceps": 0.05}'::jsonb
    WHEN e.category ILIKE '%спина%' AND (e.subcategory ILIKE '%широч%' OR e.name ILIKE '%подтяг%' OR e.name ILIKE '%блок%') THEN '{"lats": 0.7, "biceps": 0.2, "traps": 0.1}'::jsonb
    ...
    WHEN e.category ILIKE '%плеч%' AND (e.subcategory ILIKE '%средн%' OR e.name ILIKE '%махи%сторон%') THEN '{"mid_delt": 0.8, "rear_delt": 0.2}'::jsonb
    WHEN e.category ILIKE '%плеч%' AND (e.subcategory ILIKE '%задн%' OR e.name ILIKE '%наклон%') THEN '{"rear_delt": 0.8, "mid_delt": 0.2}'::jsonb
    WHEN e.category ILIKE '%плеч%' OR e.name ILIKE '%жим%вверх%' OR e.name ILIKE '%арнольд%' THEN '{"front_delt": 0.5, "mid_delt": 0.3, "triceps": 0.2}'::jsonb
    WHEN e.category ILIKE '%руки%' AND (e.subcategory ILIKE '%бицепс%' OR e.name ILIKE '%сгибан%рук%' OR e.name ILIKE '%молот%') THEN '{"biceps": 1.0}'::jsonb
