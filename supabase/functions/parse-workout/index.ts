...
ПРАВИЛА КАТЕГОРИЙ:
- "Отведение плеча в тренажере", "Махи в стороны" → Плечи (средняя дельта)
- "Отведение гантели назад (в наклоне)" → Плечи (задняя дельта), НЕ трицепс!
- Трицепс: "разгибание предплечья", "разгибание плеча" (overhead), "разгибания на блоке", "французский жим"
...
${normalizedText.trim()}`;
    ...
    if (parsed.exercises) {
      parsed.exercises = parsed.exercises.map((ex: Exercise) => {
        ...
        // Пост-обработка: AI может путать "Отведение плеча в тренажере" (средняя дельта) с "Отведение гантели назад" (задняя дельта)
      ...
      // Расширение подходов: "27 кг на 15 раз, 2 подхода" для отведения плеча / отведения гантели назад
      const approachMatch = inputLower.match(/(отведение плеча|отведение гантели назад)[^]*?(\d+(?:[.,]\d+)?)\s*кг\s+на\s+(\d+)\s+раз[^]*?(\d+)\s+подход(?:а|ов)?/i);
      if (approachMatch) {
        const [, , weightStr, repsStr, nStr] = approachMatch;
        ...
        if (n > 1) {
          const idx = parsed.exercises.findIndex((ex: Exercise) => {
            ...
            return (nl.includes('отведение плеча') || nl.includes('отведение гантели')) && ex.sets?.length === 1;
          });
          if (idx >= 0) {
